function helloConsole() {
    var staticText = 'Made with ❤️ by Demiweb';
    var staticUrl = '> https://demiweb.pro/';
    var hello = [`\n %c ${staticText} %c ${staticUrl} %c %c \n\n`, 'border: 1px solid #000;color: #fff; background: #3d2579; padding:5px 0;', 'color: #fff; background: #ffd800; padding:5px 0;border: 1px solid #000;', 'background: #fff; padding:5px 0;', 'color: #b0976d; background: #fff; padding:5px 0;'];

    if ((navigator.userAgent.indexOf("MSIE") != -1) || (!!document.documentMode == true)) {
        window.console.log(`${staticText} - ${staticUrl}`);
    } else if (navigator.userAgent.indexOf("Chrome") != -1) {
        var title = '%c';

        for (var i = 0; i < 12; i++) {
            title += ' ';
        }

        window.console.log.apply(console, hello);
        window.console.log(title, 'font: 36px/100% sans-serif; background-repeat: no-repeat; background-size: 100%; background-image: url(data:image/svg+xml;base64,);');
    } else {
        var title = 'demiweb';
        var banner = `\n%c${title.split('').join('%c %c')}%c\n`;
        var styler = [];

        var symbol = [
            'font: 32px/1.5 Helvetica, Arial, sans-serif',
            'text-transform: uppercase',
            'color: #fff',
            'background-color: #ffd800',
            'padding: 4px 14px',
        ].join(';');

        title.split('').forEach(() => {
            styler.push(symbol);
            styler.push('');
        });

        window.console.log(`${staticText} - ${staticUrl}`);
        window.console.log(banner, ...styler);
    }
}

helloConsole();


let clears = [...document.querySelectorAll('.input .clear')];

function clearInput() {
    if (clears.length) {
        clears.forEach((btn) => {
            btn.addEventListener('click', () => {
                btn.closest('.input').querySelector('.input-road').value = '';
            })
        })
    }
}

$('body').on('click', '.clear', function () {
    this.closest('.input').querySelector('.input-road').value = '';
});

$('body').on('click', '.remove-point', function () {
    this.closest('.point').remove();
});

$('.input-road').on('focus', function () {
    if (this.value !== '') {
        this.closest('.input').querySelector('.autocomplete').classList.add('autocomplete-visible');

    }

    // console.log(this);
});

$('.input-road').on('input', function () {
    if (this.value !== '') {
        this.closest('.input').querySelector('.autocomplete').classList.add('autocomplete-visible');
    } else {
        this.closest('.input').querySelector('.autocomplete').classList.remove('autocomplete-visible');

    }

    // console.log(this);
});

$('.add-point').on('click', function (e) {
    e.preventDefault();
    e.stopPropagation();

    let pointt = document.querySelector('.start .point').cloneNode(true);
    let point = document.createElement('div');


    point.classList.add('remove-point');
    point.innerHTML = "Видалити точку";


    pointt.classList.remove('start-point');
    pointt.classList.add('stop-point');
    pointt.appendChild(point);
    pointt.querySelector('p').innerHTML = 'Адреса зупинки';
    pointt.querySelector('input').value = '';


    document.querySelector('.start').appendChild(pointt);

});


$('body').on('click', function (e) {

    if (e.target.closest('.input')) {
        if (e.target.classList.contains('clear')) {
            $('.autocomplete').removeClass('autocomplete-visible');
        }
    } else {
        $('.autocomplete').removeClass('autocomplete-visible');

    }
});

// clearInput();


//map streets

var map = L.map('map-cont').setView([49.298715689383414, 31.573825822797485], 6.4);

L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
    maxZoom: 19,
    attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
}).addTo(map);



//изменять значения координат в массиве и передавать дальше в запуск функции построения карты
let roadPoints = [L.latLng(50.43783307251176, 30.486070787498058), L.latLng(49.22310276252109, 28.49333604743351), L.latLng(46.453584404719585, 30.732018739978994)];


function setRoad(points) {
    L.Routing.control({

        waypoints: roadPoints,
        createMarker: function(i, waypoints, n) {
            var startIcon = L.divIcon({
                iconSize: [40, 40],
                iconUrl: '../img/place2.svg',
                className: `custom-pane custom-name-${i + 1}`,

                html: `<i class='material-icons'>${i + 1}</i>`,

            });

            marker_icon = startIcon;

            var marker = L.marker(waypoints.latLng, {
                icon: startIcon,
            });
            return marker
        },

        lineOptions: {
            styles: [{color: '#0d99ff', opacity: 1, weight: 5}]
        }
    }).addTo(map);


}


//запуск карты по точкам, запущена сразу что бы показать точки, можно закрыть коментов
setRoad(roadPoints);

//map streets